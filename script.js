...
let questions = [
    ...
    {
        id: 3,
        question: "What is the correct JavaScript syntax to change the HTML content?",
        options: ["document.getElementById('p').innerHTML = 'Hello'", "document.getElement('p').innerHTML = 'Hello'", "document.getElementById('p').textContent = 'Hello'", "None of the above"],
        correct: 0
    },
    {
        id: 4,
    ...
    },
    {
        ...
        options: ["To store data on the server", "To store data in the browser persistently", "To store temporary session data", "To encrypt data"],
        correct: 1
    },
    {
        id: 8,
    ...
    },
    {
        ...
        options: ["Remote Execution and Storage Transfer", "Representational State Transfer", "Resource Element Static Transfer", "Remote Element System Transfer"],
        correct: 1
    },
    {
        id: 10,
...
function handleLogout() {
    if (confirm('Are you sure you want to logout?')) {
        ...
        if (studentNameField && studentNameField.value && completedStudents.includes(studentNameField.value)) {
...
function viewLeaderboard() {
    ...
    if (sorted.length === 0) {
        container.innerHTML = '<div class="empty-leaderboard"><p>No scores yet. Start a quiz to appear on the leaderboard!</p></div>';
    } else {
        let html = '';
                            ...
                            <tbody>
                                ${list.map((entry, i) => `
                                    <tr>
                                        ...
                                        <td>${entry.name}</td>
                                        <td>${entry.phone}</td>
                                        <td><strong>${entry.score}/${entry.maxScore}</strong></td>
...
function openPolesManager(closeOnly) {
    ...
    if (!manager) return;
    if (closeOnly) { manager.style.display = 'none'; return; }
...
function renderPolesManager() {
    ...
    if (!poles || poles.length === 0) {
        tbody.innerHTML = '<tr><td colspan="4" style="text-align:center; color:#999;">No poles configured</td></tr>';
        return;
    }

    tbody.innerHTML = poles.map((p, idx) => {
...
function savePoleForm() {
    ...
    if (!name) { alert('Enter a pole name'); return; }
    if (poleEditIndex >= 0) {
        poles[poleEditIndex] = name;
    } else {
        if (poles.includes(name)) { alert('Crew already exists'); return; }
        poles.push(name);
    }
...
function deletePole(idx) {
    ...
    if (!confirm(`Delete crew "${name}"? This will not remove participant records.`)) return;
    poles.splice(idx, 1);
    localStorage.setItem('poles', JSON.stringify(poles));
    populatePoleOptions();
    renderPolesManager();
...
function openSiteSettings(closeOnly) {
    ...
    if (!panel) return;
    if (closeOnly) { panel.style.display = 'none'; return; }
    panel.style.display = panel.style.display === 'block' ? 'none' : 'block';
    if (panel.style.display === 'block') {
        document.getElementById('siteTitleInput').value = siteSettings.title || '';
        document.getElementById('sitePrimaryColor').value = siteSettings.primaryColor || '';
    }
}
...
function saveSiteSettings() {
    ...
    document.querySelectorAll('.login-header h1').forEach(h => h.textContent = siteSettings.title);
...
function renderQuestionsManager() {
    ...
    if (!questions || questions.length === 0) {
        tbody.innerHTML = '<tr><td colspan="5" style="text-align:center; color:#999;">No questions</td></tr>';
        return;
    }
    ...
    tbody.innerHTML = questions.map((q, idx) => `
        <tr>
            ...
            <td>${q.options.map((o, i) => `<div><strong>${String.fromCharCode(65 + i)}.</strong> ${escapeHtml(o)}</div>`).join('')}</td>
...
function saveQuestionForm() {
    ...
    if (!text || !opt0 || !opt1 || !opt2 || !opt3 || isNaN(correct) || correct < 0 || correct > 3) {
...
function escapeHtml(unsafe) {
    ...
    if (savedPoles) {
        ...
        } catch (e) { console.error('Failed to parse saved poles:', e); }
    }
    ...
    if (savedSettings) {
        try {
            ...
            if (parsed && typeof parsed === 'object') siteSettings = Object.assign(siteSettings, parsed);
        } catch (e) { console.error('Failed to parse site settings:', e); }
    }
    ...
    document.documentElement.style.setProperty('--primary-color', siteSettings.primaryColor || getComputedStyle(document.documentElement).getPropertyValue('--primary-color'));
